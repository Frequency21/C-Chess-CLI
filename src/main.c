...
int main(int argc, char const *argv[]) {
    ...
    do {
        ...
        switch (status) {
            case DRAW_OFFERED:
                wprintf(
                    L"Your opponent offered a draw, do you accept it?\n"
                    "(type \"accept\" to accept, or anything else to decline "
                    "it.)\n");
                break;
            case DRAW_ACCEPTED:
            ...
            case ERR_WRONG_INPUT:
                wprintf(
                    L"Wrong input, type \"\033[1;31mhelp\033[0m\" to display "
                    L"possible commands.\n");
                break;
            case ERR_WRONG_MOVE:
        ...
        if (status == PROMOTION) {
            ...
            do {
                wprintf(
                    L"\nYour pawn is about to promote, choose a figure "
...
char *user_input(int length) {
    char *flag, *result = (char *)malloc(
                    sizeof(char) *
                    (length + 2));  // length doesn't contain '\n' and '\0'
